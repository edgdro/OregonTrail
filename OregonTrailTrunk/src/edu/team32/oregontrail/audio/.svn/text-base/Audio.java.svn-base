package edu.team32.oregontrail.audio;
...
public class Audio implements Runnable {
	 ...
	 */
	public Audio(File fileIn, Sound sound) {
		this.sound = sound;
		try {
			...
			if (numBytesRead != numBytes)
				System.out
						.println("Warning: Unable to read entire file.  Proceeding with partial file.");
	...
	/**
	 * Saves the current sound into a wav file. The format will be 44100 samples
	 * per second, 16 bits per sample, 2 channels, PCM Signed, big endian.
	 * 
	 * @param fileName
	 *            The name of the file to be written, should end with ".wav"
	 *            suffix
	 */
	 ...
	 * System.out.println("Cannot play: " + e); return; } } // Stops playing the
	 * current sound. If no sound is playing, the stop() // method returns
	 * without doing anything. public void stop() { if (clip != null) {
	 * clip.stop(); clip.flush(); clip.close(); clip = null; } }
	 */
	...
	// version but
	// doesn't use threads so it's not interruptible. But, at least it doesn't
	// use
	// clips so there is no size restriction.
	/*
	 * private SourceDataLine line = null; public void play() { try {
	 * AudioFormat format = desiredFormat; line =
	 ...
	 * System.out.println("Cannot play: " + e); return; } } public void stop() {
	 ...
	 * audio.play();
	 * 
	 * audio.play(); try { Thread.sleep(5000); } catch (InterruptedException e)
	 * { e.printStackTrace(); } System.out.println("Stopping clip");
